...
test('source booking payload is normalized while retaining its booking data', async () => {
  ...
  assert.equal(created.trips[0].route_id, 'r4');
  ...
  assert.equal(created.route_id, 'r4', 'single-departure clients still see a route and date');
...
test('a booking spans several departures and is refused as a whole', async () => {
  ...
  for (const service_date of [first, second]) {
    await app.inject({ method: 'POST', url: '/operations/deployments', payload: { boat_id: `boat-${service_date}`, route_id: 'r5', service_date, capacity: 10 } });
  }
  const created = await app.inject({
    method: 'POST', url: '/v1/bookings', payload: {
      agentId: 'a_b2c',
      trips: [{ routeId: 'r5', date: first, pax: { ad: 4 } }, { routeId: 'r5', date: second, pax: { ad: 3, chd_th: 1 } }],
    },
  });
  ...
  const seatsOn = async (date: string) => (await app.inject({ method: 'GET', url: `/v1/availability?route_id=r5&date=${date}` })).json().available_seats;
  ...
  const overflow = await app.inject({
    method: 'POST', url: '/v1/bookings', payload: {
      trips: [{ routeId: 'r5', date: first, pax: { ad: 7 } }, { routeId: 'r5', date: second, pax: { ad: 6 } }],
    },
  });
  ...
  const amended = await app.inject({
    ...
    payload: { trips: [{ routeId: 'r5', date: second, pax: { ad: 2 } }] },
  });
