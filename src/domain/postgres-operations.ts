...
import type { Booking, Deployment, SeatLock } from './operations.js';
...
export class PostgresOperationsStore {
  ...
  async capacity(routeId: string, serviceDate: string): Promise<Capacity> {
    const { rows: [row] } = await this.client().query(`SELECT
      ...
      COALESCE((SELECT SUM(allocated_pax) FROM bookings WHERE route_id = $1 AND service_date = $2 AND status = 'confirmed' AND booking_mode IS DISTINCT FROM 'charter'), 0)::int AS booked_pax,
      COALESCE((SELECT SUM(pax) FROM bookings WHERE route_id = $1 AND service_date = $2 AND status = 'confirmed' AND booking_mode = 'charter'), 0)::int AS charter_pax,
      COALESCE((SELECT SUM(pax) FROM seat_locks WHERE route_id = $1 AND service_date = $2 AND status = 'active'), 0)::int AS locked_pax`, [routeId, serviceDate]);
  ...
  private async assertCapacity(routeId: string, date: string, seats: number, charter = false): Promise<void> { await this.lockPool(routeId, date); const capacity = await this.capacity(routeId, date); const available = charter ? capacity.total_capacity - capacity.booked_pax - capacity.charter_pax - capacity.locked_pax : capacity.available_seats; if (available < seats) unavailable(); }
  ...
  async amendBooking(id: string, changes: Partial<Pick<Booking, 'route_id' | 'service_date' | 'pax'>>): Promise<Booking | undefined> {
    ...
    if (current.status === 'confirmed' && (route !== current.route_id || date !== current.service_date || seats !== current.pax)) {
      ...
      await this.client().query('UPDATE bookings SET allocated_pax = 0 WHERE id = $1', [id]);
      try { if ((await this.capacity(route, date)).available_seats < seats) unavailable(); }
      catch (error) { await this.client().query('UPDATE bookings SET allocated_pax = $2 WHERE id = $1', [id, current.pax]); throw error; }
    }
  ...
  async amendLock(id: string, changes: Partial<Pick<SeatLock, 'pax' | 'agent_id'>>): Promise<SeatLock | undefined> { const current = (await this.listLocks()).find((item) => item.id === id); if (!current) return undefined; const seats = changes.pax ?? current.pax; if (current.status === 'active' && seats !== current.pax) { await this.lockPool(current.route_id, current.service_date); await this.client().query('UPDATE seat_locks SET pax=0 WHERE id=$1', [id]); try { if ((await this.capacity(current.route_id, current.service_date)).available_seats < seats) unavailable(); } catch (error) { await this.client().query('UPDATE seat_locks SET pax=$2 WHERE id=$1', [id,current.pax]); throw error; } } const { rows: [row] } = await this.client().query('UPDATE seat_locks SET pax=$2, agent_id=$3, updated_at=now() WHERE id=$1 RETURNING *', [id,seats,changes.agent_id ?? current.agent_id ?? null]); return lock(row); }
  ...
  async allotment(routeId: string, date: string): Promise<Capacity & { route_id: string; service_date: string; deployments: Deployment[] }> { return { route_id: routeId, service_date: date, ...(await this.capacity(routeId,date)), deployments: await this.listDeployments(date,date,routeId) }; }
}
